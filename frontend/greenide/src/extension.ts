...
export class JavaDocumentSymbolProvider implements vscode.DocumentSymbolProvider {
    ...
    public provideDocumentSymbols(document: vscode.TextDocument, token: vscode.CancellationToken): Thenable<vscode.SymbolInformation[]> {
        ...
        return new Promise((resolve) => {
            ...
            for (var n1 = 0; n1 < data.methods.length; n1++) { kanzilistFULL[n1] = JSON.stringify(data.methods[n1]); }
            ...
            kanzilistIMP.forEach((i) => { if (!kanzilistIMPwD.includes(i)) { kanzilistIMPwD.push(i); } });

            // Two dimensional list of kanzi methods to find methods after implementation
            // kanzilist = [implemented][method]
            var kanzilist = [];
            for (var k = 0; k < kanzilistIMP.length; k++) { kanzilist.push([kanzilistIMP[k], kanzilistMET[k]]); }
            ...
            for (var i = 0; i < document.lineCount; i++) {
                ...
                for (var temp = 0; temp < kanzilist.length; temp++) {
                    ...
                    if (line.text.includes('import ' + kanzilist[temp][0])) { containedKanzis.push(kanzilist[temp]); }
                }
